...
const PostPage = () => {
	...
	const addPost = async (values: ICommentInput) => {
		try {
			const resp = await axiosInstance.post(
				`/posts/${params.postId}/comments`,
				values
			);
			const tempComment = comments as IComment[];
			setComments([...tempComment, resp.data]);
			setOpen(false);
		} catch (error) {
			console.log(error);
		}
	...
	useEffect(() => {
		(async () => {
			...
			const resp = await axiosInstance.get(`/posts/${params.postId}`);
			const data = resp.data as IPost;
			setPost(data);
			const userResp = await axiosInstance.get(`/users/${data.userId}`);
			setUser(userResp.data);
			const commentResp = await axiosInstance.get(
				`/posts/${params.postId}/comments`
			);
	...
	return (
		<>
			<div className="h-screen bg-gradient-to-bl from-stone-300 to-orange-300 relative overflow-auto">
				<div className="p-4">
					<div className="py-4 px-8 shadow-lg rounded bg-orange-300">
						<div>
						...
						<div className="flex justify-end mt-4">
							<Link
								to={`/users/${user?.id}`}
								className="text-md lg:text-xl font-medium text-indigo-500"
							>
								{user?.username}
							</Link>
						</div>
					...
					<div className="mt-2">
						...
						{comments &&
							comments
								.sort((a, b) => b.id - a.id)
								.map((comment) => (
									<div className="bg-stone-300 rounded px-4 py-2 m-2">
